...
class AIAdoptionData(models.Model):
    """Model for storing AI adoption survey data"""
    email_domain = models.CharField(max_length=100, null=True, blank=True)
    faculty = models.CharField(max_length=100, default="Unknown")
    level_of_study = models.CharField(max_length=100, default="Unknown")
    ai_familiarity = models.IntegerField(default=3)  # 1-5 scale
    uses_ai_tools = models.CharField(max_length=10, default="no")  # yes/no
    tools_used = models.TextField(null=True, blank=True)  # comma-separated list
    usage_frequency = models.CharField(max_length=20, default="never")  # never, rarely, monthly, weekly, daily
    challenges = models.TextField(null=True, blank=True)
    suggestions = models.TextField(null=True, blank=True)
    improves_learning = models.CharField(max_length=10, default="no")  # yes/no/maybe
    ...
    def __str__(self):
        return f"{self.faculty} - {self.level_of_study} - {self.adoption_level}"
...
class AIInsight(models.Model):
    ...
    source_data = models.TextField(null=True, blank=True)  # JSON formatted source data
    chart_data = models.TextField(null=True, blank=True)  # JSON formatted chart data
    chart_type = models.CharField(max_length=50, null=True, blank=True)  # bar, line, pie, etc.
    is_generated = models.BooleanField(default=False)  # Whether generated by AI
...
class InsightTopic(models.Model):
    """Model for organizing AI insights by topic"""
    name = models.CharField(max_length=100)
